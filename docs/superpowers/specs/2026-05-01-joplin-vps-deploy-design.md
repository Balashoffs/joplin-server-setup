# Дизайн: развёртывание Joplin Server на VPS (example.com)
...
Развернуть на VPS self-hosted Joplin Server с TLS-доступом по адресу `https://example.com`. Стек должен быть воспроизводим (идемпотентные скрипты), переносим (всё в git-репозитории) и быть «хорошим соседом» для других веб-приложений на том же VPS.
...
├── nginx/
│   └── example.com.conf # минимальный 80-блок, certbot достроит 443
└── scripts/
...
POSTGRES_PASSWORD=<генерируется deploy.sh при первом запуске: openssl rand -base64 24>
POSTGRES_DB=joplin
```
...
Включается **вручную** после того как пользователь убедился что HTTPS работает корректно. В `README.md` отдельный раздел с командой:
...
`deploy.sh` HSTS не трогает. Преждевременное включение HSTS на ошибочной конфигурации может на год заблокировать домен в браузерах — сознательно делаем это отдельным осознанным действием.

### 7.4 Автообновление

Пакет `certbot` Ubuntu при установке регистрирует systemd-таймер `certbot.timer` (срабатывает дважды в сутки). `--deploy-hook "systemctl reload nginx"` сохраняется в `/etc/letsencrypt/renewal/example.com.conf` и применяется при каждом продлении.
...
**Защита от self-lockout.** До любых действий с UFW скрипт определяет фактический порт sshd:

```bash
ACTUAL_SSH_PORT=$(ss -tlnp 2>/dev/null | awk '/sshd/ {split($4,a,":"); print a[length(a)]; exit}')
```

Если `ACTUAL_SSH_PORT` определился и не совпадает с `SSH_PORT` — скрипт падает с явной ошибкой и инструкцией поправить переменную. UFW не трогается до устранения расхождения.
...
7. Скопировать `nginx/example.com.conf` в `/etc/nginx/sites-available/`, создать симлинк в `sites-enabled/` (если ещё нет). `nginx -t && systemctl reload nginx`.
8. Если сертификата ещё нет (`certbot certificates | grep -q example.com` → false) — запустить certbot (см. 7.2). Иначе пропустить.
9. Финальные проверки:
   - `curl -fsS https://example.com/api/ping` → 200
   - `curl -sIo /dev/null -w '%{http_code}' http://example.com/` → 301
   - `certbot certificates` показывает `example.com` с expiry > 60 дней
...
6. Если `date +%u` = `7` (воскресенье):
   ```bash
   cp "/var/backups/joplin/joplin-${TS}.dump.gz" \
      "/var/backups/joplin/weekly-$(date +%Y-%m-%d).dump.gz"
   ```
7. Ротация:
...
Принимает аргументом путь к `.dump.gz` (`DUMP="$1"`). Интерактивно требует подтверждения (`y/N`) — деструктивно.

Шаги:

1. Проверить что `$DUMP` задан, файл существует и проходит `gunzip -t`.
2. Запросить подтверждение пользователя (`read -p "..."`).
3. `set -a; source .env; set +a` — подгрузить `POSTGRES_USER`, `POSTGRES_DB`.
4. `docker compose stop app`.
5. Сбросить схему: `docker compose exec -T db psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "DROP SCHEMA public CASCADE; CREATE SCHEMA public;"`.
6. Накатить дамп: `gunzip -c "$DUMP" | docker compose exec -T db pg_restore -U "$POSTGRES_USER" -d "$POSTGRES_DB" --no-owner --no-acl`.
7. `docker compose start app`.
8. Проверка ping: `curl -fsS --retry 30 --retry-delay 2 http://127.0.0.1:22300/api/ping`.
...
4. curl -fsS https://example.com/api/ping
```
...
| `/etc/nginx/sites-available/example.com.conf` + симлинк | deploy | конфиг nginx |
| `/etc/letsencrypt/live/example.com/` | certbot | сертификат (общая инфра certbot) |
...
1. `https://example.com` открывается в браузере, загружается web UI Joplin.
2. `curl -fsS https://example.com/api/ping` возвращает 200.
3. `curl -sI http://example.com/ | head -1` показывает `301 Moved Permanently`, `Location: https://example.com/`.
4. SSL Labs (или `testssl.sh`) для домена показывает A или выше.
5. `certbot certificates` показывает `example.com` с expiry > 60 дней.
...
9. Joplin desktop клиент успешно подключается (`Synchronisation target` = `Joplin Server`, URL = `https://example.com`) и синхронизирует тестовую заметку.
